 ...
 *
 * @author Marcin Peczarski <_____@mimuw.edu.pl>
 ...
 * ruchu oraz czy jest przynajmniej jedno pole zajęte przez innego gracza,
 * którego przejęcie nie spowodowałoby przekroczenia limitu obszarów w grze
 * (@p areas) przez żadnego z graczy.
 * @param[in] g       – wskaźnik na strukturę przechowującą stan gry,
 * @param[in] player  – numer gracza, liczba dodatnia niewiększa od wartości
 *                      @p players z funkcji @ref gamma_new.
 * @return Wartość @p true, jeśli gracz jeszcze nie wykonał w tej rozgrywce
 * złotego ruchu i jest przynajmniej jedno pole na które mógłby go wykonać,
...
/** @brief Zwraca numer gracza zajmującego dane pole.
 * @param[in] g       – wskaźnik na strukturę przechowującą stan gry.
 * @param[in] x       – numer kolumny, liczba nieujemna mniejsza od wartości
 *                      @p width z funkcji @ref gamma_new,
 * @param[in] y       – numer wiersza, liczba nieujemna mniejsza od wartości
 *                      @p height z funkcji @ref gamma_new.
 * @return Numer gracza zajmującego (@p x, @p y), liczba dodatnia;
 * 0 jeżeli dane są niepoprawne lub pole jest wolne.
 */
